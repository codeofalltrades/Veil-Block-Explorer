...
router.get("/blocks", function (req, res, next) {
    ...
    coreApi.getBlockchainInfo().then(function (getblockchaininfo) {
        ...
        if (sort === "desc") {
            ...
            for (var ig = offset; ig < offset + limit; ig++) {
                if (ig >= 0) {
                    blockHeights.push(ig);
                }
            }
        }
...
router.post("/search", function (req, res, next) {
    ...
    if (query.length > 0 && query.length <= 10) {
        return res.redirect("/block-height/" + rawCaseQuery);
    }

    if (rawCaseQuery.length == 34 || rawCaseQuery.length == 42) {
    ...
    if (query.length === 64) {
        coreApi.getRawTransaction(query).then(function (tx) {
            if(tx.hash !== undefined){
                return res.redirect("/tx/" + tx.hash);
            } else{
                coreApi.getBlockByHash(query).then(function (blockByHash) {
                    if (blockByHash.hash != undefined) {
                        return res.redirect("/block/" + blockByHash.hash);
                    }
                    else{
                        req.session.userMessage = "No results found for  " + query;
                        return res.redirect("/");
                    }
                });
            }
...
router.get("/address/:address", function (req, res, next) {
    ...
    coreApi.getAddress(address).then(function (validateaddressResult) {
                            ...
                            coreApi.getRawTransactionsWithInputs(txids).then(function (rawTxResult) {
                                ...
                                Promise.all(blockHeightsPromises).then(function () {
                                    ...
                                    for (var i = 0; i < rawTxResult.transactions.length; i++) {
                                        ...
                                        for (var ij = 0; ij < tx.vin.length; ij++) {
                                            var txInput = txInputs[ij];
                                            var vinJ = tx.vin[ij];
        ...
        res.locals.userMessage = "Failed to load address " + address + " ";
